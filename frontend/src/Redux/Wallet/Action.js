...
export const loadDemoBalance = (jwt) => async (dispatch) => {
    dispatch({ type: types.LOAD_DEMO_BALANCE_REQUEST });
  
    try {
      const response = await api.put("/api/wallet/demo/load", null, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });
      ...
      dispatch({
        type: types.LOAD_DEMO_BALANCE_SUCCESS,
        payload: response.data,
      });
    } catch (error) {
      dispatch({
        type: types.LOAD_DEMO_BALANCE_FAILURE,
        error: error.message,
      });
    }
  };

export const manualDeposit = ({ jwt, amount }) => async (dispatch) => {
    dispatch({ type: types.DEPOSIT_MONEY_REQUEST });
  
    try {
      const response = await api.put(`/api/wallet/deposit/amount/${amount}`, null, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });
      ...
      // Refresh wallet and transactions after deposit
