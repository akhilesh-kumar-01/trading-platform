...
export const toggleDemoMode = (jwt) => async (dispatch) => {
  try {
    const response = await api.patch(`/api/users/demo-mode`, null, {
      headers: {
        Authorization: `Bearer ${jwt}`,
      },
    });
    const user = response.data;
    dispatch({ type: actionTypes.TOGGLE_DEMO_MODE, payload: user.demoAccount });
    console.log("demo mode toggled ", user.demoAccount);
  } catch (error) {
    console.log("toggle demo error ", error);
  }
};
