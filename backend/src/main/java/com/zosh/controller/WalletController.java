...
//    }
    ...
    @PutMapping("/api/wallet/demo/load")
    public ResponseEntity<Wallet> loadDemoBalance(@RequestHeader("Authorization") String jwt) throws Exception {
        User user = userService.findUserProfileByJwt(jwt);
        Wallet wallet = walleteService.loadDemoBalance(user);
        return new ResponseEntity<>(wallet, HttpStatus.OK);
    }
}
